...
public sealed class InventoryGridCollectionElement : VisualElement
{
    ...
    public Vector2Int ScreenPositionToGrid(Vector2 position)
    {
        ...
        // Clamp into grid bounds to avoid negative/out-of-range indices caused by borders/scroll
        var gridWidth = Mathf.Max(1, Mathf.RoundToInt(PixelGridSize.x / _cellSize));
        var gridHeight = Mathf.Max(1, Mathf.RoundToInt(PixelGridSize.y / _cellSize));
        gridPosition.x = Mathf.Clamp(gridPosition.x, 0, gridWidth - 1);
